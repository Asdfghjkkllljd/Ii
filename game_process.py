...
class GameProcess:
    ...
    async def dice_procces(self, message: Message, type, n = None):
        ...
        if type == "number":
            ...
            if n == v:
                await func.winner(message, self.amount, self.asset, self.coef, self.id, self.username,  f"Победа! Выпало число {v}.", photo="win.jpg", type='c')
            else:
    ...
    async def duel_number_process(self, message: Message, num, game = '🎲', textes = ["первого кубика", "второго кубика"]):
        ...
        while True:
            ...
            if msguser.dice.value > msgbot.dice.value:
                ...
                await message.reply("⚡ Ничья! Играем ещё раз!", photo="dice_even.jpg", type='c')
    ...
    async def duel_proccess(self, message: Message, game = '🎲', textes = ["первого кубика", "второго кубика"], win_photos = ['win.jpg','dice_2.jpg']):
        self.coef += coefs.DUEL
        while True:
